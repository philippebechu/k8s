...
DOCKER_EXACT_VERSION=$(sudo apt-cache madison docker-ce | grep $DOKER_MAJOR_VERSION | head -1 | awk '{print $3}') 

echo $DOCKER_EXACT_VERSION

sudo apt-get install -y docker-ce=$DOCKER_EXACT_VERSION docker-ce-cli=$DOCKER_EXACT_VERSION containerd.io
...
K8S_EXACT_VERSION=$(sudo apt-cache madison kubectl | grep $K8S_MAJOR_VERSION | head -1 | awk '{print $3}')

echo $K8S_EXACT_VERSION

sudo apt-get install -y kubelet=$K8S_EXACT_VERSION kubeadm=$K8S_EXACT_VERSION kubectl=$K8S_EXACT_VERSION
